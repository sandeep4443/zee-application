                ...
                {selectedItems && selectedItems.length > 0 &&
                    <div>
                        <h2 className={headerClass}>
                            <strong className="text-secondary">Selected Items:</strong>
                        </h2>
                        {selectedItems.map(
                            item => (
                ...
                <div className="row d-flex flex-row py-5">
                    <div className="w-100 px-4 py-5 d-flex flex-row flex-wrap align-items-center justify-content-between">
                        <div className="d-flex flex-row align-items-center">
                            <h2 className={headerClass}><strong className="text-secondary">{totalMovies}</strong>Movies</h2>
                            {currentPage && (
                                <span className="current-page d-inline-block h-100 pl-4 text-secondary">Page
                    ...
                    {currentMovies.map((movie, index) => {
                        return (
                            <MovieCard {...this.props} value={index} movie={movie}/> 
                        )
                    }
                    )}
                </div>
